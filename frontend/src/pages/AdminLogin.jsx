...
export default function AdminLogin() {
  ...
  useEffect(() => {
    if (!window.FB) {
      ...
      script.onload = () => window.FB.init({ appId: import.meta.env.VITE_FACEBOOK_APP_ID || '', version: 'v19.0', xfbml: true })
  ...
  const handleGoogle = () => {
    ...
    window.google.accounts.id.initialize({
      ...
      callback: async (response) => {
        try {
          setSubmitting(true)
          const data = await authApi.googleLogin({ credential: response.credential, role: 'admin' })
          setAuthDirect(data)
          navigate('/', { replace: true })
        } catch (err) {
          setError(err.message)
  ...
  const handleFacebook = () => {
    if (!window.FB) {
      setError('Facebook is loading. Please try again.')
      return
    }
    window.FB.login(async (fbResponse) => {
      ...
      try {
        setSubmitting(true)
        const data = await authApi.facebookLogin({ accessToken: fbResponse.authResponse.accessToken, role: 'admin' })
        setAuthDirect(data)
        navigate('/', { replace: true })
      } catch (err) {
        setError(err.message)
      ...
      >
        <div className="flex items-center gap-2">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-teal">
            <UserCog className="h-4.5 w-4.5 text-ink" strokeWidth={2.5} />
          </div>
        ...
        <p className="mt-1 font-sans text-sm text-ink/50 dark:text-paper/50">
          Sign in to manage your assigned PG.
        </p>
